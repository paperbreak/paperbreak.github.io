...
This post is a breakdown of the paper [Breaking the Softmax Bottleneck: A High-Rank RNN Language Model](https://arxiv.org/abs/1711.03953)
...
\begin{aligned}
    & \text{Given } A_1 \neq A_2 \in F(A) \\
    & A_1 = A + \Lambda_1 \mathbb{J}_{m,n} \\
    & A_2 = A + \Lambda_2 \mathbb{J}_{m,n} \\
    & \implies A_1 = A_2 + (\Lambda_1 - \Lambda_2)\mathbb{J}_{m,n} \\
    & \implies \text{rank}(A_1) = \text{rank}(A_2 + (\Lambda_1 - \Lambda_2)\mathbb{J}_{m,n}) \\
    & \phantom{\implies} \phantom{\text{rank}(A_1)} \leq \text{rank}(A_2) + \text{rank}((\Lambda_1 - \Lambda_2)\mathbb{J}_{m,n}) \\
    & \phantom{\implies} \phantom{\text{rank}(A_1)} \leq \text{rank}(A_2) + \min(\text{rank}(\Lambda_1 - \Lambda_2),\text{rank}(\mathbb{J}_{m,n})) \\
    & \phantom{\implies} \phantom{\text{rank}(A_1)} \leq \text{rank}(A_2) + 1 \\
    & \text{similarly, we can find that } \text{rank}({A_2}) \leq \text{rank}(A_1) - 1 \\
    & \implies \vert \text{rank}(A_1) - \text{rank}(A_2) \vert \leq 1 \quad \blacksquare \\
\end{aligned}
$$
